...
- **Final transcription:** WhisperKit (MLX-optimized for Apple Silicon) batch processing after recording stops
...
- **Hybrid transcription: SFSpeech live + WhisperKit batch** — `SpeechStreamingTranscriber` wraps `SFSpeechRecognizer` for near-instant word-by-word live text during recording. WhisperKit runs batch transcription after stop for the final stored transcript. This replaced the previous `StreamingTranscriber` which ran WhisperKit every 10 seconds on a sliding window (slow, chunky updates). SFSpeech handles 48kHz stereo float32 directly — no resampling needed. WhisperKit no longer loads during recording (~1.5GB RAM saved). `StreamingTranscriber.swift` left in place but unused.

...
- **Google Calendar integration** — App-owned OAuth client (embedded credentials, PKCE flow). Users click "Sign in with Google" — no Cloud project setup required. `CalendarService.findCurrentMeetingWithFallback()` tries EventKit first, falls back to Google Calendar API. Tokens stored in Keychain. Participants included in LLM summarization prompt and displayed in meeting detail views. `GoogleCalendarConfig` is an enum with static constants; `GoogleCalendarAuthService` handles OAuth with a self-retaining localhost callback server (`LocalOAuthCallbackServer`).

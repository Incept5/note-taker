...
- **Microphone capture:** AVAudioEngine with configurable gain, mixed into system audio via ring buffer
...
- **Mic mixed into system audio** — AVAudioEngine captures mic into a thread-safe ring buffer (`MicRingBuffer`), mixed into SCStream callback before writing to file with configurable gain (`micGain`, default 2.0x, range 0.5x–5.0x via Settings slider). Produces a single combined M4A with all voices. Voice processing (AEC) is intentionally NOT enabled — it attenuated the system-wide mic signal, making users inaudible to other meeting participants. Some mic echo may appear in recordings but is acceptable for transcription. Don't use `captureMicrophone = true` on macOS 15+ — it didn't deliver audio reliably.
...
- **Structured summary output** — LLM prompted for JSON with distinct fields (overview, key decisions, action items, discussion highlights by topic, blockers, next steps). `MeetingSummary` has backward-compatible decoding: old summaries (with `summary`, `keyPoints`, `decisions`, `openQuestions`) decode via optional fields and display through computed fallback properties (`effectiveOverview`, `effectiveKeyDecisions`, etc.). `DiscussionTopic` struct holds `topic` + `detail` for each discussion highlight.
- **Custom system prompt** — Users can edit the summarization system prompt via Settings → Summarization → Edit Prompt. `SummarizationService.defaultSystemPromptTemplate` holds the built-in prompt with `{{duration}}`, `{{context}}`, `{{participants}}` placeholders. `AppState.customSystemPrompt` stores the override (persisted in UserDefaults via `setCustomSystemPrompt(_:)` — uses an explicit method instead of `didSet` to avoid `@Published` init bugs that wipe UserDefaults). Passed to `SummarizationService.customSystemPrompt` before each summarization run. When nil/empty, the default template is used.
- **Transcript content gate** — `transcriptHasSufficientContent()` in `AppState` checks that a transcript has at least 50 words with >15% unique word ratio before sending to the LLM. Prevents hallucination on silence, noise, or very short recordings. Summarization is silently skipped (meeting stays at `.transcribed` phase with its transcript intact).
- **Custom MLX models** — `MLXModelManager` supports adding/removing HuggingFace models by ID or URL. Custom models persisted in UserDefaults (`customMLXModels` key). Curated models can be hidden (`removedMLXModelIds` key). `MLXModel` struct has `isCustom` flag. Settings UI includes text field with paste button and auto-download on add.
...
- **Hybrid transcription: SFSpeech live + WhisperKit fallback** — `SpeechStreamingTranscriber` wraps `SFSpeechRecognizer` for near-instant word-by-word live text during recording. The transcriber accumulates text across session resets: `textBuffer` holds committed text from previous sessions, `sessionLatest` holds the current session's partial. `emitUpdate()` always sends `textBuffer + sessionLatest` as the full text. Within-session text shrinks (SFSpeech silently resets without error 209) are detected in `handleResult` and trigger `commitSession()` to preserve text before it's overwritten. `AppState.bufferLiveText()` simply stores the full text as `currentChunkText` and updates the display — no segment creation during recording. On stop, `commitCurrentChunk()` creates a single segment from the accumulated text. `startTranscriptionWithStreamingSegments()` passes through the transcript without requiring a WhisperKit model. WhisperKit is only used as a fallback when SFSpeech produced no segments. SFSpeech handles 48kHz stereo float32 directly — no resampling needed. WhisperKit no longer loads during recording (~1.5GB RAM saved). `StreamingTranscriber.swift` left in place but unused.
- **SFSpeech session restart** — Apple's `SFSpeechRecognizer` times out after ~60 seconds (error 209) or silently resets within a session (text shrinks without any error). `SpeechStreamingTranscriber` handles both: `sessionID` counter ignores stale callbacks, `textBuffer`/`sessionLatest` pattern accumulates confirmed text, and within-session shrink detection (`text.count < sessionLatest.count / 2`) commits text before it's overwritten. `AtomicReference<T>` (NSLock-protected box) gives `nonisolated appendBuffer` thread-safe access to the current `SFSpeechAudioBufferRecognitionRequest`.
